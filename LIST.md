...
- [ ] ПРОТОКОЛ ИЗОЛЯЦИИ (по нему ищем вектор): А — зайти ВООБЩЕ без экзекьютора на 3–5 мин
  (кикает = флаг аккаунта/сервера, скрипт ни при чём, ждать 12–24 ч); B — открыть экзекьютор,
  НИЧЕГО не выполнять (кикает = артефакт самого экзекьютора, сменить/спрятать его оверлей);
  C — выполнить наш скрипт, НЕ вызывать GUI, подождать 2 мин (кикает при чистых A/B = остался
  след от нас, смотреть «[mh] ok» в консоли F9); D — вызвать GUI и подождать (кикает только тут =
  GUI-детект, усилить камуфляж);
- [ ] если A/B/C чистые, а D кикает — перевести GUI в не-PlayerGui маунт всегда и разбить окно;
