...
- [ ] проверить сборку 0.6.0 в игре: загрузка → простоит 2–3 минуты БЕЗ включения функций →
  кика быть не должно (если кик повторяется даже на GHOST — дело в самом экзекьюторе,
  попробовать другой: Delta-альтеры/Codex/Arceus X, слабые Xeno/Solara палится сразу);
